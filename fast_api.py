...
def create_account(account_number: str,account_name: str, balance: float):
    try:
        return account_services.create_account(account_number,account_name,Decimal(balance))
    except Exception as e:
        raise HTTPException(status_code=400)

@app.get("/accounts/{account_number}")
def get_account(account_number: str):
        account = transaction_services.get_account(account_number)
        if not account:
...
def deposit(account_number: str,amount: float):
     try:
          return{
               "new_balance": transaction_services.deposit(account_number,Decimal(amount))
          }
     except Exception as e:
          raise HTTPException(status_code=400)
...
def withdraw(account_number: str,amount: float):
     try:
          return{
               "new_balance": transaction_services.withdraw(account_number,Decimal(amount))
          }
     except Exception as e:
          raise HTTPException(status_code=400)
...
def transfer(from_account :str , to_account: str, amount: float):
     try:
          return transaction_services.transfer(from_account, to_account, Decimal(amount))
     except Exception as e:
          raise HTTPException(status_code=400)
