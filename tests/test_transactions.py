...
def test_deposit(transaction_setup):
    ...
    new_balance = trans_mnger.deposit("ACC2001",Decimal("200.00"))
    assert new_balance == Decimal("700.00")

def test_deposit_nonexistent_account(transaction_setup):
    _, trans_mnger = transaction_setup
    with pytest.raises(ValueError, match="Account not found"):
        trans_mnger.deposit("ACC999", Decimal('100.00'))

def test_deposit_invalid_amount(transaction_setup):
    account, trans_mnger = transaction_setup
    account.create_account("ACC2003","Olivia Davis",Decimal("400.00"))
    with pytest.raises(ValueError, match="Amount must be greater than zero"):
        trans_mnger.deposit("ACC2003",Decimal("-50.00"))
...
def test_withdraw_and_deposit(transaction_setup):
    account, trans_mnger = transaction_setup
    account.create_account("ACC2005","Sophia Wilson",Decimal("1000.00"))
    trans_mnger.deposit("ACC2005",Decimal("500.00"))
